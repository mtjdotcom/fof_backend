...
def ingest_portfolio_history(file_path):
    ...
    try:
        meta = {
            ...
            # NEW: Load the Master Fund table for the lookup
            'master_funds': pd.read_sql("SELECT * FROM isomer_funds", engine)
        }
    except Exception as e:
        print(f"⚠️ Warning: Metadata load failed ({e}).")
        meta = {
            'urls': pd.DataFrame(), 'names': pd.DataFrame(), 
            'tags': pd.DataFrame(), 'funds': pd.DataFrame(),
            'master_funds': pd.DataFrame()
        }

    print("Running cleaning logic...")
    ...
    print(f"✅ Successfully ingested {len(final_df)} rows.")
