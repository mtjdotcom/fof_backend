...
def seed_metadata():
    ...
    try:
        ...
        if os.path.exists("data/company_urls_master.csv"):
            pd.read_csv("data/company_urls_master.csv", encoding='utf-8-sig').rename(columns={'LPA Num': 'lpa_num', 'Organization URL': 'url'}).to_sql('meta_urls', engine, if_exists='replace', index=False)
